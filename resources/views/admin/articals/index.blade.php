                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <img src="{{ URL::asset('storage/articals/' . $artical->image) }}" class="img-fluid"
                                    style="height:50px;">
                                @else
                                NA
                                @endif
                                 ...
                                 <td>  @if($artical->aproved == 'active')<a href="{{url('admin/article/sts/block/'. $artical->id)}}"><button
                                        type="button" class="btn btn-success">{{ $artical->aproved }}</button></a>
                                        @elseif($artical->aproved == 'block')
                                        <a href="{{url('admin/article/sts/active/'. $artical->id)}}"><button
                                        type="button" class="btn btn-danger">{{ $artical->aproved }}</button></a></td>
                                @endif
                            ...
                            <td class="text-truncate">
                                ...
                                <a href="{{ route('articals', $artical->url) }}" target="blank" class="text-success"
...
function deleteConfirmation(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: `{{ URL::to('admin/manage-artical/${id}') }}`,
                type: "DELETE",
                dataType: "json",
...
$(document).ready(function() {
    ...
    $(document).on("click", ".add-artical", function(event) {
        $.ajax({
            url: "{{ URL::to('admin/manage-artical/create') }}",
            type: "GET",
            dataType: "json",
            success: function(result) {
                if (result.success) {
    ...
    $(document).on("click", "#add-artical-btn", function(event) {
        ...
        formData.append('image', (typeof $('#image')[0].files[0] == 'undefined') ? '' : $('#image')[
            0].files[0]);
        $.ajax({
            url: "{{ URL::to('admin/manage-artical') }}",
            type: 'POST',
            processData: false,
            contentType: false,
            dataType: 'json',
    ...
    $(document).on("click", ".edit-artical", function(event) {
        let id = $(this).attr('artical_id');
        $.ajax({
            url: `{{ URL::to('admin/manage-artical/${id}/edit') }}`,
            type: "get",
            dataType: "json",
            success: function(result) {
                if (result.success) {
    ...
    $(document).on("click", "#update-artical-btn", function(event) {
        // for (instance in CKEDITOR.instances) {
        //  CKEDITOR.instances[instance].updateElement();
        //}
        $(this).attr('disabled', true);
        ...
        formData.append('status', $('#status').val());
        formData.append('author_id', $('#author_id').val());
        formData.append('aproved', $('#aproved').val());
        formData.append('date', $('#date').val());
        formData.append('image', (typeof $('#image')[0].files[0] == 'undefined') ? '' : $('#image')[
            0].files[0]);
        let artical_id = $(this).attr('artical_id');
        $.ajax({
            url: `{{ URL::to('admin/manage-artical/${artical_id}') }}`,
            type: 'POST',
            processData: false,
            contentType: false,
            dataType: 'json',
