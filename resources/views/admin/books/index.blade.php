                ...
                <table class="table table-striped table-bordered" id="for_all">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <a href="{{ asset('storage/files/'.$book->book_pdf) }}" class="text-success" title="view" target="_blank"><i class="fa fa-eye" aria-hidden="true"></i></a>
...
<script>
    function deleteConfirmation(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: `{{ URL::to('admin/manage-book/${id}') }}`,
                    type: "DELETE",
                    dataType: "json",
    ...
    $(document).ready(function(event) {
        ...
        $(document).on("click", ".add-book", function(event) {
            $.ajax({
                url: "{{ URL::to('admin/manage-book/create') }}",
                type: "GET",
                dataType: "json",
                success: function(result) {
                    if (result.success) {
        ...
        $(document).on("click", ".add-book-btn", function(event) {
            ...
            formData.append('book_image', (typeof $('#image')[0].files[0] == 'undefined') ? '' : $(
                '#image')[0].files[0]);
            formData.append('book_cover', (typeof $('#book_cover')[0].files[0] == 'undefined') ? '' : $(
                '#book_cover')[0].files[0]);
            formData.append('book_file', (typeof $('#book_file')[0].files[0] == 'undefined') ? '' : $(
                '#book_file')[0].files[0]);
            $.ajax({
                url: "{{ URL::to('admin/manage-book') }}",
                type: 'POST',
                processData: false,
                contentType: false,
                dataType: 'json',
        ...
        $(document).on("click", ".edit-book", function(event) {
            ...
            $.ajax({
                url: `{{ URL::to('admin/manage-book/${id}/edit') }}`,
                type: "get",
                dataType: "json",
                success: function(result) {
                    if (result.success) {
        ...
        $(document).on("click", "#update-book-btn", function(event) {
            ...
            $.ajax({
                url: `{{ URL::to('admin/manage-book/${book_id}') }}`,
                type: 'POST',
                processData: false,
                contentType: false,
                dataType: 'json',
